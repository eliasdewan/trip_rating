...
export function modernHtml(requestData: Array<{ entry: string, data: string }>, resultData: Array<{ entry: string, data: string }>) {
  ...
  parsedResultData.forEach((result, index) => {
    ...
    resultHtml +=
      <div style={`border:1px solid ${color}; box-shadow: 10px 5px 5px red; border-radius:10px; margin:10px 0; background:#fafbfc; box-shadow:0 2px 8px #0001;`}>
        <div style="padding:16px; display:flex; flex-direction:column; gap:8px;">
          <div >
            <span style={`font-size:1.5em; color:white; background-color:${color}; border-radius: 5px; padding: 5px 5px; margin:0;`}>  {application ?? '-'}  </span>
        </div>
